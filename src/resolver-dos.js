import { Main } from "./scripts/main-dos.js";
import { Graph } from "./scripts/algorithms/prim.js";
import { WeightedGraph } from "./scripts/algorithms/dijkstra.js";
import { Link, Node } from "./scripts/elements/index.js";
import { onReset } from "./scripts/fm-main.js";
import { fordFulkerson } from "./scripts/algorithms/max-flow.js";
...
export class Resolver {
  ...
  runForPrimAndDijkstra = () => {
    ...
    primOption?.addEventListener("click", (e) => {
      ...
      if (isChecked) {
        this.resolveBy.push("Prim");
        return;
      }

      this.resolveBy = this.resolveBy.filter((resolve) => resolve !== "Prim");
    });
    ...
    resolveButton?.addEventListener("click", () => {
      ...
      if (!this.resolveBy.includes("Prim") && existentPrimSolution) {
        existentPrimSolution.remove();
      }
      ...
      if (this.resolveBy.includes("Dijkstra")) {
        this.dijkstraSolution = this.resolveByDijkstra();
        if (existentDijkstraSolution) {
          existentDijkstraSolution.remove();
        }
  ...
  resolveByPrim = () => {
    ...
    const g = new Graph(this.main.nodes.length);
    edges.map((edge) => {
    ...
    this.mstPrimSolution = g.primsMST();
  };
  ...
  resolveByDijkstra = () => {
    ...
    if (this.main.acceptedNodes.length < 2) {
      console.log("NO hay nodos con estado aceptado");
      let sortedNodes = this.main.nodes
